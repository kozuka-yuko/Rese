...
class ShopRepControllerTest extends TestCase
{
    ...
    public function it_redirects_to_confirm_page_with_image_upload()
    {
        ...
        $response = $this->post(route('shopCreateConfirm'), [
            ...
            'area' => 1,
            'genre' => 1,
            'description' => 'ぜひいらしてください。',
        ]);

        $response->assertRedirect(route('shopCreateConfirm'));

        $this->assertNotNull(Session::get('form_input'));

        $sessionData = session('form_input');

        $this->assertEquals('Test Shop', $sessionData['name']);
        $this->assertNotNull($sessionData['img_url']);
        $this->assertEquals(1, $sessionData['area_id']);
        $this->assertEquals(1, $sessionData['genre_id']);
        $this->assertEquals('ぜひいらしてください。', $sessionData['description']);
        $this->assertStringContainsString('images/', $sessionData['img_url']);
    }
}
